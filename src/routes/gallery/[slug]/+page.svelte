<script>
	...
	let knifeExists = true;
	async function fetchData() {
		const knives = await Data.getGallery();
		knife = knives.find((item) => item._id === data.slug);
		if (knife) {
...
{:else}
	<div class="text-center mx-auto p-4">
		{#if !knifeExists}
			<h1 class="text-4xl font-bold mb-4">Knife Not Found</h1>
			<a href="/gallery" class="text-oglala underline">Return to Gallery</a>
		{:else}
